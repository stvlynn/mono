...
export async function runAnthropicMessagesConversation(options: LlmRunOptions): Promise<ConversationMessage[]> {
  ...
  for (let step = 0; step < options.maxSteps; step += 1) {
    const assistantMessage = await runAnthropicStepWithRetry(options, conversation, streamState);
    ...
    const toolMessages = await executeAnthropicToolCalls(options, toolCalls, scheduler, toolResultMap, streamState);
...
async function runAnthropicStepWithRetry(
  options: LlmRunOptions,
  messages: ConversationMessage[],
  streamState: AnthropicStreamState
): Promise<AssistantMessage> {
  ...
  while (true) {
    try {
      return await runAnthropicStep(options, messages, streamState);
    } catch (error) {
      const normalized = normalizeProviderError(error);
      const safeToRetry = normalized.retryable
      ...
      if (!safeToRetry) {
        writeDevRetryLog(
          options,
          `request failed after attempt=${attempt} kind=${normalized.kind} retryable=${normalized.retryable} saw_assistant=${streamState.sawAssistantDelta} saw_reasoning=${streamState.sawReasoningDelta} saw_tool_call=${streamState.sawToolCall} executed_tool=${streamState.executedTool} code=${normalized.code ?? "-"} status=${normalized.statusCode ?? "-"} message=${normalized.message}`
        );
        throw error;
      }

      const delayMs = computeRetryDelay(attempt, normalized, DEFAULT_RETRY_POLICY);
      writeDevRetryLog(
        options,
        `retrying attempt=${attempt + 1} after_ms=${delayMs} kind=${normalized.kind} saw_assistant=${streamState.sawAssistantDelta} saw_reasoning=${streamState.sawReasoningDelta} saw_tool_call=${streamState.sawToolCall} executed_tool=${streamState.executedTool} code=${normalized.code ?? "-"} status=${normalized.statusCode ?? "-"} message=${normalized.message}`
      );
