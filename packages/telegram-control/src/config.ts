...
export async function buildTelegramStatusResult(
  ...
  return {
    ...
    lines: [
      ...
      `Actions enabled: ${Object.entries(config.actions).filter(([, enabled]) => enabled).map(([key]) => key).join(", ") || "<none>"}`,
      ...
      `Reply multi-message: ${config.reply.multiMessage ? "yes" : "no"}`,
      `Reply split delay: ${config.reply.splitDelayMs}ms`,
      `Reply sticker store: ${config.reply.stickers.storePath}`,
