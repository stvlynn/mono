...
export function renderEditorSection(options: EditorSectionOptions): string[] {
  ...
  lines.push(`  ${ansi.dim("/help /profile /model /auth /sessions /skills /context /memory /tree /attach /detach /attachments /quit | Ctrl+J newline | Ctrl+L profiles | Ctrl+R sessions")}`);
...
export function renderModal(modal: ModalState, width: number): string[] {
  if (modal.type === "help") {
    return [
      ...
      padRight("  /skills    Browse available skills", width),
      ...
      padRight("  /attach    Attach an image by path", width),
      padRight("  /detach    Remove a pending attachment", width),
      padRight("  /attachments Show pending attachments", width),
